...
- UI 呈现：展示输入信息 + 压缩比率设置 + “开始压缩”
  - 若检测到视频 BPP 过低（已深度压缩），显示“视频已高度压缩”警告。
- 允许操作：
  - 调整滑杆/点击预设（更新当前目标比例）
  - 点击开始压缩
  - 重新选择文件（覆盖）
...
- 默认：**音频流复制（Copy）**。
  - 理由：避免音频重编码带来的音质损失与额外耗时，且音频通常占体积比例较小（相比视频）。
...
- 实际实现逻辑：
  - **Ratio < 40%** 且当前长边 > 1280 时：缩放至长边 1280px。
  - **40% ≤ Ratio < 60%** 且当前长边 > 1920 时：缩放至长边 1920px (1080p)。
  - **Ratio ≥ 60%**：保持原分辨率。
...
- 实际实现逻辑：**Target Bitrate Mode (目标码率模式)**。
  - 算法：`Target Bitrate = (Input Size * Ratio) / Duration * 0.9` (预留 10% 给容器/音频)。
  - 动态码率上限：根据视频时长限制最大码率（30s内 30Mbps -> >5min 5Mbps），防止浏览器内存崩溃。
  - 速度优先：使用 `-preset ultrafast`。
...
- 实际实现逻辑：**混合进度估算**。
- 逻辑：优先使用 FFmpeg `progress` (0-1)，若无效则基于 `time / duration` 估算。
- 限制：进度条最大限制在 99% 直到文件完全写入完成。
